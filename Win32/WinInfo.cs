...
namespace XJK.Win32
{
    public class WinInfo
    {
        ...
        public bool IsForeground => Foreground.Get() == this;
        ...
        public WinInfo(IntPtr handle)
        {
            Handle = handle;
        }

        public WinInfo(Window window)
        {
            Handle = new WindowInteropHelper(window).Handle;
        }

        public static implicit operator IntPtr(WinInfo win) => win.Handle;
        public static implicit operator WinInfo(IntPtr handle) => new WinInfo(handle);

        // Static methods

        public static IntPtr GetForegroundHandle() => GetForegroundWindow();

        public static WinInfo GetForeground() => new WinInfo(GetForegroundHandle());

        public static WinInfo Find(string? CaptionName, string? ClassName = null)
        {
            if (CaptionName is null && ClassName is null)
                return GetForeground();

            var ptr = FindWindow(ClassName ?? "", CaptionName ?? "");
            return new WinInfo(ptr);
        }

        public static WinInfo Find(string Title)
        {
